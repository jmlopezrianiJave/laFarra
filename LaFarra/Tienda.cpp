...
void Tienda::productosEscasos(int n){
	string lista = "\n\nProductos con menos de " + std::to_string(n) + " unidades: \n";
	for(map<int, Producto>::iterator pProducto = inventario.begin(); pProducto != inventario.end(); pProducto++){
		if(pProducto->second.getCantUnidades() < n){
			lista += "-" + pProducto->second.getNombre() + "\n";
		}
	}
	lista += "\n\n";
	cout<<lista;
...
void Tienda::productosVendidosTipo(){
	int snacks, licores;
	for(vector<Factura>::iterator pFactura = facturas.begin();
		 pFactura != facturas.end(); pFactura++){
			 for(vector<DetalleFactura>::iterator pDetalleFactura = pFactura->getDetalles().begin();
		 pDetalleFactura != pFactura->getDetalles().end(); pFactura++){
			 if(pDetalleFactura->getProducto().getTipoProducto().getNombre() == "snack"){
				 snacks += pDetalleFactura->getCantUnidVend();
			 }
			 else{
				 licores += pDetalleFactura->getCantUnidVend();;
			 }
		 }
	}
...
void Tienda::productosEnTienda(){
	string productos = "\n\nProductos disponibles: \n";
	for(map<int, Producto>::iterator pProducto = inventario.begin(); pProducto != inventario.end(); pProducto++){
		productos += "-" + pProducto->second.getNombre() + "\n";
	}
}

float Tienda::IVAaDIAN(){
	float total;
	for(vector<Factura>::iterator pFactura = facturas.begin(); pFactura != facturas.end(); pFactura++){
			total += pFactura->getvalorTotalIVA();
	}
	return total;
}

void Tienda::totalGanado(){
	...
	for(vector<Factura>::iterator pFactura = facturas.begin(); pFactura != facturas.end(); pFactura++){
			for(vector<DetalleFactura>::iterator pDetalleFactura = pFactura->getDetalles().begin(); pDetalleFactura != pFactura->getDetalles().end(); pFactura++){
				costo = pDetalleFactura->getProducto().getCosto() * pDetalleFactura->getProducto().getCantUnidades();
		 	}
		ganancia += pFactura->getValorPagarSinIVA() - costo;
	}
	cout<<"Las ganacias totales de la tienda hasta el momento son: "<<ganancia<<"\n\n";
}
